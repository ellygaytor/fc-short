checksum = function (message) {
	var c = 0;
	var byte_array = message.split("").map(function (x) {
		return x.charCodeAt(0);
	});
	var table = [
		,
	];

	for (var i = 0; i < byte_array.length; i++) {
		c = table[(c ^ byte_array[i]) % 256];
	}

	return ("00" + c.toString(16)).substr(-2);
};

function encode(code) {
	code = code.replace(/[a-z]/gi, "");
	code = code.replace(/-/g, "");
	code = parseInt(code, 10).toString(32);
	code += checksum(code);
	code = code.toUpperCase();
	return code;
}

function decode(code) {
	code = code.toLowerCase();
	if (checksum(code.slice(0, -2)) != code.substr(-2)) {
		return false;
	}
	code = code.substr(0, code.length - 2);
	code = parseInt(code, 32).toString(10);
	code = "SW" + code;
	code =
		code.substring(0, 2) +
		"-" +
		code.substring(2, 6) +
		"-" +
		code.substring(6, 10) +
		"-" +
		code.substring(10, 14);
	return code;
}

function createCode() {
	var code = encode(document.getElementById("code-input").value);
	var url = window.location.href;
	var url = url.substring(0, url.lastIndexOf("/"));
	document.getElementById("url-output").value = url + "/" + code;
	document.getElementById("pre").style.display = "none";
	document.getElementById("post").style.display = "block";
	var qrcode = new QRCode(document.getElementById("qrcode"), {
		text: url + "/" + code,
		width: 128,
		height: 128,
		colorDark: "#000000",
		colorLight: "#ffffff",
		correctLevel: QRCode.CorrectLevel.H,
	});
	document.getElementById("qrcode").style.display = "block";
}

document.getElementById("checksum-error").style.display = "none";
document.getElementById("qrcode").style.display = "none";

var parts = document.location.pathname.split("/");
var id = parts[1];
if (id.length > 0) {
	document.getElementById("encode").style.display = "none";

	var id = decode(id);
	if (id != false) {
		document.getElementById("code").value = id;
		document.getElementById("code-display").style.display = "block";
	} else {
		document.getElementById("code-display").style.display = "none";
		document.getElementById("checksum-error").style.display = "block";
	}
} else {
	document.getElementById("code-display").style.display = "none";
	document.getElementById("encode").style.display = "block";
	document.getElementById("pre").style.display = "block";
	document.getElementById("post").style.display = "none";
}

function copyFriend() {
	var copyText = document.getElementById("code");
	copyText.select();
	copyText.setSelectionRange(0, 99999);
	document.execCommand("copy");
}
function copySelf() {
	var copyText = document.getElementById("url-output");
	copyText.select();
	copyText.setSelectionRange(0, 99999);
	document.execCommand("copy");
}
